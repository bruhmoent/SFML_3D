...
void MathObject::connect_face(const std::vector<size_t>& face_points, const std::vector<std::vector<float>>& rotation_matrix) {
    ...
    for (size_t i = 0; i < face_points.size(); ++i) {
        ...
        m_lines.append(sf::Vertex(sf::Vector2f(p1_projected.x, p1_projected.y), sf::Color(0,0,0,125)));
        m_lines.append(sf::Vertex(sf::Vector2f(p2_projected.x, p2_projected.y), sf::Color(0,0,0,125)));
    }
}
